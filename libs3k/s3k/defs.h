...
typedef enum { S3K_R = 1, S3K_RW = 3, S3K_RX = 5, S3K_RWX = 7 } s3k_perm_t;
...
static inline s3k_cap_t s3k_mktime(uint64_t hartid, uint64_t base,
				   uint64_t size)
{
	s3k_cap_t cap;
...
static inline int s3k_is_parent(s3k_cap_t a, s3k_cap_t b)
{
	...
	if (a.type == S3K_CAPTY_MONITOR && b.type == S3K_CAPTY_MONITOR) {
		return a.mon.base <= b.mon.base
		       && (b.mon.base + b.mon.size)
			      <= (a.mon.base + a.mon.size);
	}

	if (a.type == S3K_CAPTY_CHANNEL && b.type == S3K_CAPTY_CHANNEL) {
		return a.chan.base <= b.chan.base
		       && (b.chan.base + b.chan.size)
			      <= (a.chan.base + a.chan.size);
	}
	...
	if (a.type == S3K_CAPTY_SOCKET && b.type == S3K_CAPTY_SOCKET) {
		return a.sock.channel == b.sock.channel && a.sock.tag == 0
		       && b.sock.tag != 0;
	}
