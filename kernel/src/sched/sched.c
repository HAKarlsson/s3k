...
proc_t *schedule_get(uint64_t hartid, uint64_t current_time,
		     uint64_t *start_time, uint64_t *end_time)
{
	...
	*start_time
	    = ((current_time + SCHEDULER_TIME) / SLOT_LENGTH) * SLOT_LENGTH;
...
void schedule_update(uint64_t pid, uint64_t end, uint64_t hartid, uint64_t from,
		     uint64_t to)
{
	for (uint64_t i = from; i < to; ++i) {
		_frames[i][hartid - MIN_HARTID]
		    = (frame_info_t){ .pid = pid & 0xFF, .end = end & 0xFF };
	}
...
retry: // Get the quantum.
	...
	proc_pmp_load_to_hardware(current);
